...
//    }
    ...
    private void updateTerm(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd"); // Điều chỉnh định dạng theo yêu cầu
        Date startDate;
        Date endDate;
    ...
    private void deleteTerm(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (isDeleted) {
            request.setAttribute("message", "Term deleted successfully!");
        } else {
            request.setAttribute("message", "Failed to delete the term.");
        }
        response.sendRedirect("term");  // Redirect to the term list after deletion
    }
